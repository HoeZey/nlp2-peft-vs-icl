...
def train(model_type, config={}):
    if model_type not in models:
        raise ValueError(
            f"Model {model_type} not found. Available models: {list(models.keys())}"
        )
    ...
    train_dataset, val_dataset = (
        dataset["train"].train_test_split(test_size=0.01).values()
    )
    ...
    for i in range(num_examples):
        ...
        output = model.generate(
            **tokenizer(prompt, return_tensors="pt").to(model.device), do_sample=False
        )
